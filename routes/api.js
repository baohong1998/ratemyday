...
router.post("/create-account", (req, res) => {
  ...
  bcrypt.genSalt(saltRounds, (err, salt) => {
    bcrypt.hash(password, salt, (err, hash) => {
      pool.query(
        "INSERT INTO user_credentials (username, email, phone, password) VALUES (?,?,?,?)",
        ...
        (errors, results, fields) => {
          if (errors) {
            ...
            pool.query(
              "INSERT INTO user_info (id, firstname, lastname, gender, dateofbirth) VALUES ((SELECT id FROM user_credentials WHERE username = ?),?,?,?,?)",
              [
                username.trim(),
                first.trim(),
                last.trim(),
...
router.post("/authenticate", (req, res) => {
  ...
  pool.query(
    "SELECT id, password FROM user_credentials WHERE username = ? OR email = ?",
    [username, username],
    (errors, results, fields) => {
      if (errors) {
        console.log(errors);
...
router.get("/get-user-info", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    //console.log(user_id)
    pool.query(
      "SELECT user_info.*, user_credentials.username, user_credentials.email, user_credentials.phone FROM user_info, user_credentials WHERE user_credentials.id = ? AND user_info.id = ?",
      [user_id, user_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
...
router.patch("/edit-user-info", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "UPDATE user_info SET firstname = ?, lastname = ?, gender = ?, dateofbirth = ? WHERE id = ?",
      [first.trim(), last.trim(), gender.trim(), birthday.trim(), user_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
          return res.json({
          ...
          pool.query(
            "UPDATE user_credentials SET username = ?, email = ?, phone = ? WHERE id = ?",
            [username.trim(), email.trim(), phone.trim(), user_id],
            (errors, results, fields) => {
              if (errors) {
                console.log(errors);
                return res.json({
                ...
                if (
                  ...
                  oldUsername !== email.trim()
                ) {
                  let newToken = jwt.sign(
                    { username: username.trim(), id: user_id },
                    process.env.TOKEN_KEY
                  );
                  return res.json({
                    success: true,
...
//rating
router.get("/check-today-post", (req, res) => {
  const token = req.headers.authorization.split(" ")[1];
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    if (err) {
      return res.status(401).end();
    ...
    const user_id = decoded.id;
    pool.query(
      "SELECT id, rating, avg_public_rate, reasons, public FROM personal_rating WHERE DATE(timestamp) = DATE(CURDATE()) AND user_id = ?",
      [user_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
...
router.post("/post-personal-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "SELECT * FROM personal_rating WHERE user_id = ? AND DATE(timestamp) = DATE(NOW())",
      [user_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
          ...
          if (results.length > 0) {
            ...
            pool.query(
              "INSERT INTO personal_rating (user_id, rating, reasons, timestamp, public, edited) VALUES (?,?,?,?,?, 0)",
              [user_id, rating, reasons, timestamp, public],
              (errors, results, fields) => {
                if (errors) {
                  console.log(errors);
                  return res.json({
...
router.patch("/edit-personal-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "UPDATE personal_rating SET rating = ?, reasons = ?, timestamp = ?, public = ?, edited = 1 WHERE id = ? AND user_id = ?",
      [rating, reasons, timestamp, public, post_id, user_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
          return res.json({
...
router.get("/get-personal-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "SELECT * FROM personal_rating WHERE user_id = ? AND month(timestamp) = ?",
      [user_id.month],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
...
router.get("/get-public-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    pool.query(
      "SELECT * FROM public-rating WHERE post_id = ?",
      [post_id],
      (errors, results, fields) => {
        if (errors) {
          return res.json({
...
router.post("/post-friend-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "INSERT INTO public_rating (post_id,friend_id, friend_rating, comment) VALUES (?,?,?,?)",
...
router.patch("/edit-friend-rating", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "UPDATE public_rating SET friend_rating = ?, comment = ? WHERE post_id = ? AND friend_id = ?",
      [rating, comment, post_id, user_id],
      (errors, results, fields) => {
        if (errors) {
          return res.json({
            success: false,
...
router.get("/get-public-list", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "SELECT friend_id FROM friendship WHERE user_id = ? AND status = 3",
      [user_id],
      (errors, results, fields) => {
        if (errors) {
          return res.json({
        ...
        console.log(tup);
        pool.query(
          "SELECT * FROM personal_rating WHERE user_id IN (" +
            tup +
            ") AND public = 1",
          (errors, results, fields) => {
            if (errors) {
...
router.get("/search-user", (req, res) => {
  ...
  pool.query(
    "SELECT user_credentials.id, user_credentials.username, user_info.firstname, user_info.lastname FROM user_credentials INNER JOIN user_info ON user_credentials.id = user_info.id WHERE user_credentials.username LIKE ? OR user_credentials.email LIKE ? OR user_info.firstname LIKE ? OR user_info.lastname LIKE ?",
...
router.post("/friend-request", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    const user_id = decoded.id;
    pool.query(
      "SELECT uuid FROM friendship WHERE user_id = ? AND friend_id = ?",
      [user_id, friend_id],
      (errors, results, fields) => {
        if (errors) {
          console.log(errors);
        ...
        if (results.length > 0) {
          ...
          pool.query(
            "INSERT INTO friendship (uuid,user_id, friend_id, status) VALUES (?,?,?,0)",
            [relation_id, user_id, friend_id],
            (errors, results, fields) => {
              if (errors) {
                console.log(errors);
                return res.json({
              ...
              pool.query(
                "INSERT INTO friendship (uuid,user_id, friend_id, status) VALUES (?,?,?,1)",
                [relation_id, friend_id, user_id],
                (errors, results, fields) => {
                  if (errors) {
                    console.log(errors);
                    return res.json({
...
router.patch("/request-response", (req, res) => {
  ...
  return jwt.verify(token, process.env.TOKEN_KEY, (err, decoded) => {
    ...
    if (response == 2) {
      pool.query(
        "DELETE FROM friendship WHERE uuid = ?",
        [relation_id],
        (errors, results, fields) => {
          if (errors) {
            console.log(errors);
    ...
    pool.query(
      "UPDATE friendship SET status = ? WHERE uuid = ?",
